...
fai = fread(fai_file, col.names = c('accession.version', 'genome_len', 'a', 'b', 'c'), sep = '\t')
gi_taxid = fread(gi_taxid_file, col.names = c('accession','accession.version','taxid','gi'))
output = args[[3]]
...
taxid_len <- merge(fai[, c('accession.version', 'genome_len')],
                   gi_taxid, by = 'accession.version')
